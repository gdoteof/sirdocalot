...
const TOOLS = [
...

  {
    name: "define_widget",
    description:
      "Add a widget to the shared library. Widgets are data — a props declaration plus a layout " +
      "template — so one you define here is usable in the very next create_brief call, with no deploy " +
      "and no restart. Define one when list_widgets has nothing that fits; reaching for raw blocks " +
      "repeatedly is the signal.\n\n" +
      "The template language is exactly three constructs:\n" +
      '  {"$":"path"} substitutes the value at that prop path, whatever its type.\n' +
      '  {"$each":"path","as":"item","body":[...]} repeats body once per array element; inside it ' +
      '{"$":"item"} is the element and {"$":"item_index"} its 0-based position.\n' +
      '  {"$if":"path","then":[...],"else":[...]} includes one branch or the other.\n' +
      "Anything else in the layout is copied through as a primitive block.\n\n" +
      "`example` is a set of sample props that make the layout render. The service expands the layout " +
      "with it before storing, and refuses the definition if the result is not valid blocks — so a " +
      "broken widget fails here rather than mid-brief. Built-in widgets cannot be redefined.",
    inputSchema: {
      type: "object",
      properties: {
        name: { type: "string", description: "Widget name, referenced later as {\"widget\":\"<name>\"}." },
        summary: { type: "string", description: "One line saying when to reach for this widget." },
        props: {
          type: "array",
          description: "The props this widget accepts. An undeclared prop is refused at use time.",
          items: {
            type: "object",
            properties: {
              name: { type: "string" },
        ...
        layout: {
          type: "array",
          minItems: 1,
          description: "Template nodes: primitive blocks, with $ / $each / $if where props are substituted.",
          items: {},
        },
        example: {
          type: "object",
          description: "Sample props that make the layout render. The definition is refused if they do not.",
        },
      },
      required: ["name", "summary", "props", "layout", "example"],
      additionalProperties: false,
    },
    async run(args) {
      const body = {
        ...
        props: Array.isArray(args.props) ? args.props : [],
        layout: requireArray(args, "layout"),
        example: args.example ?? {},
      };
      if (typeof body.example !== "object" || body.example === null || Array.isArray(body.example)) {
        throw new Refusal('"example" must be an object of sample props.');
      }

      const result = await apiJson("POST", "/api/widgets", body);
      if (!result.ok) return result;
      return { ok: true, text: `Widget defined: ${pretty(result.data)}` };
    },
  },
];
